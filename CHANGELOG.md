...
### Fixed
- **Current OpenAI prices.** The OpenAI table ended at `gpt-4o` / `o1`, so newer models were unpriced — reported at $0.00, and under a cost cap raising `UnpricedModelError`. It now covers the standard-tier prices for gpt-6-astra, gpt-5.6, gpt-5.5, gpt-5.4, gpt-5.3-codex, gpt-5.2, gpt-5.1, gpt-5 (and mini / nano / pro), gpt-4.1, o3, o3-mini, o3-pro, o4-mini, and o1-pro, from OpenAI's pricing page on 2026-09-21. Every -pro model is listed, so none falls through to its base model's price. Cached input bills at the model family's real discount (0.1x for gpt-5 / gpt-6, 0.25x for gpt-4.1 / o3 / o4-mini) instead of a flat 0.5x. The server's OTLP pricing matches.
